                ...
                <div>
                    <form action="{{ route('filterBalance') }}" method="POST" class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <input type="date" class="form-control" name="start" value="{{ now()->format('Y-m-d') }}"
                                id="">
                        </div>
                        <div>
                            <label style="font-size: 14px;" for="start">End Date</label>
                            <input type="date" class="form-control" name="end" value="{{ now()->format('Y-m-d') }}"
                                id="">
                        </div>
                        <div class="pt-4">
                            <button type="submit" id="filter"><i class="fa fa-filter" aria-hidden="true"></i></button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <section class="sec-second-income">
                <div class="container">
                    ...
                    <div class="d-flex justify-content-center mt-3">
                        <button id="inc-btn"
                            class="btn @if ($balance->account_type == '0') active @endif rounded-pill me-1">Income</button>
                        <button id="exp-btn"
                            class="btn @if ($balance->account_type == '1') active @endif rounded-pill me-1">Expense</button>
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                @empty
                                    <li><a href="/account">Add Acount</a></li>
                                @endforelse
                                <li>
                    ...
                    <form id="form-data" action="{{ route('balance', [$balance->account_id, $balance->id]) }}"
                        method="POST">
                        @csrf
                        <div>
                            <input type="hidden" name="account_id" value="{{ $data->account_id }}">
                            <input type="hidden" class="" name="account_type"
                                value="@if ($balance->account_type == '0') 0 @else 1 @endif" id="data">
                        </div>
                        <div class="form-group">
                            <label class="mb-3" for="amount">
                                @if ($balance->account_type == '0')
                                    Income
                                @else
                                    Expense
                                @endif Edit
                ...
                @enderror
                                value="{{ $balance->amount }}" id="amount" class="form-control mb-3"
                                placeholder="Enter @if ($balance->account_type == '0') Income @else Expense @endif ...">
                            @error('amount')
                    ...
                    style="border: 1px solid red;"
                @enderror
                                value="{{ $balance->note }}" name="note" class="form-control mb-3"
                                placeholder="Enter notes for @if ($balance->account_type == '0') Income @else Expense @endif..">
                            @error('note')
                                <span class="text-danger t-8">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" value="{{ $balance->date }}" name="date" class="form-control mb-3"
                ...
                @enderror>
                    ...
                    style="border: 1px solid red;"
                @enderror
                                value="{{ $balance->time }}" name="time" class="form-control mb-3"
                                placeholder="Select Time...">
                            @error('time')
                                <span class="text-danger t-8">{{ $message }}</span>
                            @enderror
                        </div>
                        <button type="submit" id="update">Update @if ($balance->account_type == '0')
                                Income
                            @else
                                Expense
                            @endif
